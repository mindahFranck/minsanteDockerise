import type { Model, ModelStatic, FindOptions, WhereOptions, Transaction } from "sequelize"
import { NotFoundError } from "../utils/ApiError"
import { BaseRepository, type QueryOptions } from "../repositories/BaseRepository"
...
export class BaseService<T extends Model> {
  ...
  async withTransaction<R>(callback: (t: Transaction) => Promise<R>): Promise<R> {
  ...
  async paginate(
    ...
    const cacheKey = `${this.cachePrefix}:paginate:${page}:${limit}:${JSON.stringify(options)}`
